== A systematic mapping study: The new age of software architecture from monolithic to microservice architecture—awareness and challenges

#block(fill: rgb("f8f9fa"), inset: 8pt, radius: 3pt, width: 100%)[
  #text(size: 0.9em)[*Source:* A. Razzaq and S. A. K. Ghayyur "A systematic mapping study: The new age of software architecture from monolithic to microservice architecture—awareness and challenges," _Computer Applications in Engineering Education_, 2023. [Online]. Available: #link("https://onlinelibrary.wiley.com/doi/10.1002/cae.22586")]
]

#block(fill: rgb("f8f9fa"), inset: 8pt, radius: 3pt, width: 100%)[
  #text(size: 0.9em)[*Paper Information:* Systematic Mapping Study (SMS)]
]

#block(fill: rgb("f8f9fa"), inset: 8pt, radius: 3pt, width: 100%)[
  #text(size: 0.9em)[*Rating:* ⭐⭐⭐⭐⭐]
]